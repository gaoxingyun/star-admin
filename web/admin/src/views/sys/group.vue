<template>
  <div class="app-container calendar-list-container">
    ...
    <div class="filter-container">
      <el-input style="width: 200px;" class="filter-item" placeholder="组编号" v-model="listQuery.id" @keyup.enter.native="handleQuery">
      </el-input>
...
-->
    ...
    <el-table :data="list" v-loading.body="listLoading" element-loading-text="拼命加载中" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column label="组编号"  align="center">
        <template scope="scope">
          {{scope.row.id}}
        </template>
      </el-table-column>
      ...
      <el-table-column label="组详情"  align="center">
        <template scope="scope">
          <span>{{scope.row.detail}}</span>
        </template>
      </el-table-column>
       <el-table-column label="父组编号"  align="center">
        <template scope="scope">
          {{scope.row.parent_id}}
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form class="small-space" :model="group" :rules="rules" label-position="left" label-width="70px" style='width: 400px; margin-left:50px;'>        

        <el-form-item label="组名" prop="name">
          <el-input v-model="group.name" :disabled="dialogStatus!='create'" placeholder="请输入组名">
          </el-input>
        </el-form-item>

         <el-form-item label="组描述" prop="detail">
          <el-input v-model="group.detail" :disabled="dialogStatus!='create' && dialogStatus!='update'" placeholder="请输入组描述">
          </el-input>
        </el-form-item>

        <el-form-item label="父组编号" prop="parent_id">
          <el-input v-model="group.parent_id" :disabled="dialogStatus!='create' && dialogStatus!='update'" placeholder="请输入父组编号">
          </el-input>
        </el-form-item>
...
<script>
import { getList,createGroup,deleteGroup,updateGroup,queryGroup } from '@/api/sys/group'
...
export default {
  data() {
    return {
      ...
      rules: {
          ...
          parent_id: [
            { required: true, message: '请输入父组编号', trigger: 'blur' }
          ],
      },
    };
  ...
  methods: {
    ...
    remove(){
      deleteGroup(this.group).then(response => {
        if(response.code = 200){
          this.dialogFormVisible = false 
          this.handleFilter()
        }
    ...
    query(){
      queryGroup(this.listQuery).then(response => {
        if(response.code = 200){
          this.listLoading = true
          this.list = response.data.items
          this.listLoading = false
