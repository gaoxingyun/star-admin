<template>
  <div class="app-container calendar-list-container">
    ...
    <div class="filter-container">
      <el-input style="width: 200px;" class="filter-item" placeholder="角色编号" v-model="listQuery.id" @keyup.enter.native="handleQuery">
      </el-input>
...
-->
    ...
    <el-table :data="list" v-loading.body="listLoading" element-loading-text="拼命加载中" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column label="角色详情"  align="center">
        <template scope="scope">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form class="small-space" :model="role" :rules="rules" label-position="left" label-width="70px" style='width: 400px; margin-left:50px;'>        

        <el-form-item label="角色名" prop="name">
          <el-input v-model="role.name" :disabled="dialogStatus!='create'" placeholder="请输入角色名">
          </el-input>
        </el-form-item>

         <el-form-item label="角色描述" prop="detail">
          <el-input v-model="role.detail" :disabled="dialogStatus!='create' && dialogStatus!='update'" placeholder="请输入角色描述">
          </el-input>
        </el-form-item>
      </el-form>
...
<script>
import { getList,createRole,deleteRole,updateRole,queryRole } from '@/api/sys/role'
...
export default {
  ...
  methods: {
    ...
    remove(){
      deleteRole(this.role).then(response => {
        if(response.code = 200){
          this.dialogFormVisible = false 
          this.handleFilter()
        }
